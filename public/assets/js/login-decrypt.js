...
async function runKeyDecryption() {
  ...
  try {
    ...
    await fetch('/public/api/e2ee/clear-login-data.php', {
      method: 'POST',
    ...
    setTimeout(async () => {
      try {
        await fetch('/public/api/e2ee/clear-keys.php', { method: 'POST' });
      } catch (e) { }
      window.location.href = '/public/pages/setup-keys.php';
    }, 2000);
  }
}
...
function showKeyError(message) {
  ...
  setTimeout(() => { window.location.href = '/public/pages/signin.php'; }, 3000);
}
