...
const GatherlyE2EEChat = (function () {
  ...
  async function getOrCreateSessionKey(conversationId, otherUserId) {
    ...
    const response = await fetch(`/public/api/e2ee/get-public-key.php?userId=${otherUserId}`);
  ...
  async function sendMessage(message, recipientUserId) {
    try {
      ...
      const statusResponse = await fetch(`/public/api/e2ee/key-status.php`);
      const statusData = await statusResponse.json();

      const recipientStatusResponse = await fetch(`/public/api/e2ee/get-public-key.php?userId=${recipientUserId}`);
